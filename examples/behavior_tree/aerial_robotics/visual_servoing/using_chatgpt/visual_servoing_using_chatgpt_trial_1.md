...
Procedure:
{
    Do phase 1: {  // Approach the basketball
        Do phase 1-1: {
            "object_in_view"  // Check if the basketball is in view
        } Do phase 1-2 if 1-1 success: {
            Do phase 1-2-1: {
                "linear_z_control"  // Adjust drone's altitude to match the basketball's altitude
            }
            Do phase 1-2-2: {
                "linear_x_control"  // Move drone along the X-axis to match the basketball's horizontal position
            }
            Do phase 1-2-3: {
                "linear_y_control"  // Move drone along the Y-axis to match the basketball's vertical position
            }
        }
    }
    
    Do phase 2 if 1-2 success: {  // Report distance error and proceed to next step
        Do phase 2-1: {
            "report_error"  // Report the current distance error to supervisor
        }
    }
}
```
...
![](/examples/figs/chatgpt/visual_servoing_trial_1.png)
